...
| admin | Quản lý user, gán role, khoá/mở tài khoản | USER, STAFF, REFEREE, JOCKEY |
| staff | Season, Racecourse, Meeting, Race, Condition, Prize; duyệt registration; xác nhận entry | SEASON, RACE_MEETING, RACE, RACE_CONDITION, PRIZE_STRUCTURES, RACE_REGISTRATION, RACE_ENTRY |
| referee | Nhập kết quả race; viết báo cáo vi phạm | RACE_RESULT, REFEREE_REPORT |
| owner | Quản lý ngựa; nộp đăng ký đua; gửi lời mời jockey | HORSE, RACE_REGISTRATION, RACE_INVITATION |
...
Component dùng chung **≥ 2 role** → `components/shared/` (chống lặp ở luồng xuyên role): `RegistrationForm`, `RegistrationTable`, `EntryTable`, `RaceResultTable`, `HorseProfileCard`.

**Core workflow (theo ERD — AI phải tôn trọng thứ tự):**
```
SEASON → RACE_MEETING → RACE → RACE_REGISTRATION → RACE_INVITATION → RACE_ENTRY → RACE_RESULT
```
Một "registration" đi xuyên nhiều role: owner nộp → staff duyệt → owner mời jockey → jockey nhận → staff xác nhận entry → đua → referee nhập kết quả + báo cáo. Vì xuyên role, UI/logic chung của luồng này phải ở `components/shared/` + `services/`, **KHÔNG copy-paste** vào từng `pages/<role>/`.
...
```
RACE_REGISTRATION:  PENDING → APPROVED | REJECTED
RACE_INVITATION:    SENT → ACCEPTED | DECLINED
RACE_ENTRY:         chỉ được tạo khi  REGISTRATION = APPROVED  VÀ  INVITATION = ACCEPTED
RACE_RESULT:        chỉ nhập được khi RACE = COMPLETED
RACE:               UPCOMING → ONGOING → COMPLETED
```

Quy tắc bắt buộc:
- ❌ Staff **KHÔNG** tạo Entry trực tiếp — Entry chỉ sinh ra khi đủ điều kiện trên.
