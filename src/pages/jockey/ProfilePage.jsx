...
export default function JockeyProfilePage() {
  ...
  useEffect(() => {
    jockeyProfileService.getByUser(user.id)
      .then((p) => {
        setProfile(p);
      ...
      <Row className="g-4">
        <Col lg={7}>
          <div className="dash-card d-flex flex-column gap-4">
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 20 }}>
              <SectionTitle>Thông tin Jockey</SectionTitle>
              <Row className="g-3">
                <Col sm={6}>
                  <ReadonlyField label="Cân nặng (kg)" value={profile?.weight != null ? `${profile.weight} kg` : null} />
                </Col>
                <Col sm={6}>
                  <ReadonlyField label="Kinh nghiệm (năm)" value={profile?.experienceYears != null ? `${profile.experienceYears} năm` : null} />
                </Col>
              </Row>
            </div>
          </div>
        ...
        <Col lg={5}>
          <div className="dash-card h-100">
            ...
            <SummaryRow label="Cân nặng" value={profile?.weight != null ? `${profile.weight} kg` : null} />
            <SummaryRow label="Kinh nghiệm" value={profile?.experienceYears != null ? `${profile.experienceYears} năm` : null} />
          </div>
        </Col>
      </Row>
