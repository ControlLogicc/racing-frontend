  ...
  return (
    <div className="lux-panel" style={{ height: '100%' }}>
      ...
      {[
        ...
        ['🏆 Yêu cầu Class', race.classRequirement || 'Không giới hạn'],
        ['🛣️ Loại đường đua', race.trackType || '—'],
        ['👥 Số ngựa (Min - Max)', (race.minEntries != null && race.maxEntries != null) ? `${race.minEntries} - ${race.maxEntries} con` : '—'],
      ].map(([label, val]) => (
            ...
            <div className="lux-panel">
              ...
              >
                ...
                <Form.Group>
                  ...
                  <Form.Select value={horseId} onChange={(e) => setHorseId(e.target.value)} required>
                    ...
                    {horses.map((h) => (
                      <option key={h.id} value={h.id}>
                        {h.name}{h.horseClass ? ` (Class ${h.horseClass})` : ''}{h.rating != null ? ` — Điểm: ${h.rating}` : ''}
                      </option>
                    ))}
                  </Form.Select>
                  {horses.length === 0 && (
                  ...
                  {horseId && selectedHorse && selectedRace?.classRequirement && (
                    <Form.Text style={{ 
                      color: selectedHorse.horseClass && selectedHorse.horseClass <= Number(selectedRace.classRequirement) ? '#4caf7d' : '#f0a500'
                    }}>
                      Ngựa: Class {selectedHorse.horseClass || 'Chưa xác định'} &mdash; Race yêu cầu: Class {selectedRace.classRequirement}
                    </Form.Text>
                  )}
                </Form.Group>
