    ...
    <div className="row g-4 mb-4">
      {items.map((item) => (
        <div className="col-12 col-sm-6 col-xl-3" key={item.id}>
          <div
            className="card h-100 shadow-sm dashboard-widget-card dashboard-stat-card"
            style={{
          ...
          >
            <div className="card-body">
              ...
              {item.hint && <small className="dashboard-stat-hint">{item.hint}</small>}
            </div>
          </div>
        </div>
      ))}
...
export function ActivityPanel({ title = 'Recent Activity', items = [] }) {
  return (
    <div
      className="card h-100 shadow-sm dashboard-widget-card dashboard-panel-card"
    ...
    >
      ...
      <ul className="list-group list-group-flush dashboard-activity-list">
        {items.length === 0 && (
      ...
      <div className="card-body d-flex flex-column gap-2">
        {actions.map((a) => (
          <Link
            key={a.id}
            to={a.to}
            className="btn text-start fw-semibold dashboard-action-link"
