...
const realService = {
  // Owner xem tất cả đăng ký của mình (PENDING, APPROVED, REJECTED...)
  getByOwner: () => 
    api.get('/registrations')
       .then((r) => mapRegs(r.data))
       .catch(() => []),

  // Owner xem riêng đăng ký đã APPROVED (dùng cho flow mời Jockey)
  getApprovedForInvitation: () =>
    api.get('/owner/registrations/approved')
       .then((r) => mapRegs(r.data))
       .catch(() => []),

  // Staff xem registrations cho race được gán
  // Backend: GET /api/staff/registrations trả về StaffRegistrationResponse[]
  getAll: async (params) => {
    try {
      return await api.get('/staff/registrations', { params }).then((r) => mapRegs(r.data));
    } catch {
      // fallback: dùng /registrations (owner endpoint — ít dữ liệu hơn) hoặc trả []
      return [];
    }
  },
